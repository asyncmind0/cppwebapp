...
User(std::string username, std::string firstname, std::string lastname, std::string password, std::tm created_datetime)
    :username(username), firstname(firstname), lastname(lastname), password(password), created_datetime(created_datetime)
    {
    }
    ~User(){
    }
    friend std::ostream& operator<<(std::ostream& ostr, const User& user){
      ostr << "username:" << user.username << std::endl;
    ...
    static int create_table(soci::session &sql){
        try{
            sql << "create table users (username varchar(10) NOT NULL PRIMARY KEY, firstname varchar(100), lastname varchar(100), password varchar(8), createdtime timestamp);";
    ...
    static int get_all(soci::session &sql, std::list<User> &users){
        try{
            std::cout << "We have " << get_row_count("users", sql) << " entries in the blog.\n";
    ...
    static int create(soci::session &sql, User& p){
        try{
            int count;
            sql << "insert into users(username, firstname, lastname, password, createdtime) values(:username, :firstname, :lastname, :password, :createdtime)", soci::use(p);
